...
echo "[TASK 1] K8S Controlplane Join - API Server 192.168.10.100" 
...
mkdir -p $HOME/.kube
sshpass -p "1234" scp -o StrictHostKeyChecking=no root@k8s-m:/etc/kubernetes/admin.conf $HOME/.kube/config >/dev/null 2>&1
